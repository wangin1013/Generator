...
<div align="center">
	...
	<s:if test='%{#status=="00"}'>
		<a href="javascript:void(0);" onclick="javascript:return disable_data('<s:property value="id" />','<%=Constants.ENABLE%>');">
			<img title="<s:text name="res.enable"/>" alt="<s:text name="res.enable"/>" src="<s:url value="/server/images/yes.gif" />" border="0" />
		</a>
		
	</s:if>
	<s:else>
		<a href="javascript:void(0);" onclick="javascript:return disable_data('<s:property value="id" />','<%=Constants.DISABLE%>');">
			<img title="<s:text name="res.disable"/>" alt="<s:text name="res.disable"/>" src="<s:url value="/server/images/stop.gif" />" border="0" />
		</a>
	</s:else>
</div>