...
function insert_click(event) {
	return insert_click_base(event,"<s:url action="datatypeInsert" />");
}
//更新数据
function update_click(event) {
	return update_click_base(event,"<s:url action="datatypeUpdate" />");
}
function update_data(id) {
	return update_data_base(id,"<s:url action="datatypeUpdate" />");
}
//查看数据
function detail_data(id) {
	return detail_data_base(id,"<s:url action="datatypeDetail" />");
}
//删除数据
function delete_click(event){
	return delete_click_base(event,"<s:url action="datatypeDelete" />");
}
function delete_data(id){
	return delete_data_base(id,"<s:url action="datatypeDelete" />");
}
//禁用，启用数据
function disable_data(id,enable){
	return disable_data_base(id,enable,"<s:url action="datatypeDisable" />");
}
//查询
function search_click(event){
	...
	return search_click_base(event,"<s:url action="datatype" />",queryCondition);
}
...
$(document).ready(function(){
	$("#view\\.searchName").focus(function(){ 
		  $("#view\\.searchName").val($("#view\\.searchName").val());
	});
	$("#view\\.searchName").focus();
});
</script>
</head>
<body>
<s:form id="insertForm" action="datatype"></s:form>
<s:form id="listForm" action="datatype">
				...
				<td>
					<table>
						...
						<tr>
							...
							<td>
								<s:select id="view.searchStatus" name="view.searchStatus" list="view.statusMap" emptyOption="true"></s:select>
							</td>
...
<s:fielderror key="deleteError" cssStyle="color: red;" />
<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
	<tr>
		<td height="30px">
