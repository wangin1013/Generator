...
function insert_click(event) {
	return insert_click_base(event,"<s:url action="basedataInsert" />");
}
//更新数据
function update_click(event) {
	return update_click_base(event,"<s:url action="basedataUpdate" />");
}
function update_data(id) {
	return update_data_base(id,"<s:url action="basedataUpdate" />");
}
//查看数据
function detail_data(id) {
	return detail_data_base(id,"<s:url action="basedataDetail" />");
}
//删除数据
function delete_click(event){
	return delete_click_base(event,"<s:url action="basedataDelete" />");
}
function delete_data(id){
	return delete_data_base(id,"<s:url action="basedataDelete" />");
}
//禁用，启用数据
function disable_data(id,enable){
	return disable_data_base(id,enable,"<s:url action="basedataDisable" />");
}
//搜索
function search_click(event){
	...
	var search_datatype = document.getElementById("view.searchDatatype").value.trim();
	
	if(search_name!="") {
		queryCondition = " and baseData.name like '%" + search_name + "%'";
	}
	if(search_code) {
		queryCondition+=" and baseData.code like '%" + search_code + "%'";
	}
	
	if(search_status!="") {
		queryCondition+=" and baseData.status ='"+search_status+"'";
	}
	
	if(search_datatype!="") {
		queryCondition+=" and dataType.id='"+search_datatype+"'";
	}
	return search_click_base(event,"<s:url action="basedata" />",queryCondition);
}
...
$(document).ready(function(){
	$("#view\\.searchDatatype").focus();
});
</script>
</head>
<body>
<s:form id="insertForm" action="basedata"></s:form>
<s:form id="listForm" action="basedata">
				...
				<td>
					<table>
						<tr>
							...
							<td>
								<s:select name="view.searchDatatype" id="view.searchDatatype" emptyOption="true" list="view.datatypeList" listKey="id" listValue="name"></s:select>
							</td>
